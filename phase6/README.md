...
2. Create a new package in `gatewayserver` called `com.revature.gatewayserver.filters` > create a class called `FilterUtility.java`
...
3. Within `com.revature.gatewayserver.filters`, create a new class called `RequestTraceFilter` which implements `GlobalFilter` interface.  The class should look like this:
...
4. Now we must add these `correlation id`'s to our controllers.  Go to `AccountsController` and update the methods as follows, so that our AccountsControllerknows the trace ID that's generated by the Gateway.  Edit the `myCustomerDetails()` method and its **fallback method**:
...
5. Update the abstract methods in `CardsFeignClient` and `LoansFeignClient` to process the correlation trace ID:
...
6. Update the `CardsController` & `LoansController`  within their respective services to accept the new `@RequestHeader` since these FeignClients will call them:
...
7. Now go *back* to your `gatewayserver`'s `filter` package and add one more class called `ReponseTraceFilter`.  This class' repsonsibility is to add the trace id to the Response Header so that the client can view the trace id as well:
